...
public class UserDAOImpl implements UserDAO{
	...
	public boolean createUser2(String nombre, String apellidos, String tipoUser, String password, String email) { 
	...
	/*
	public boolean SuccessUpdateMedico (String medicoMail){
		// comprobamos que el mail pertenece a un medico registrado
		EntityManager em = EMFService.get().createEntityManager();
		Query q = em.createQuery("select item from User item where item.email = :email and item.tipoUser = :medico");
		q.setParameter("email", medicoMail);
		
		if(q.getResultList().isEmpty()){
			//el medico no esta registro o se introducido mal el email
			return false;
		} else {
			//el medico esta registrado
			return true;
		}
	}
	*/
